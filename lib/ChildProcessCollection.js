...
class ChildProcessCollection extends EventEmitter {
	...
	fork (modulePath) {
		...
		// If absolute path could not be resolved, set the module as errored.
		if (error) {
			setTimeout(() => {
			...
			// If an error didn't occur, we'll start the module and add it to the collection.
			child.start();
		...
		if (callback) {
			child.once('status-change', function (oldStatus, newStatus, error) {
				if (newStatus === 'started') {
					callback(null, child);
				} else {
					callback(error, child);
				}
			});
		}
