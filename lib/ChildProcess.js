...
class ChildProcess extends EventEmitter {
	...
	start (callback, error) {
		...
		if (callback) {
			// Bind the callback function to be executed when the process reaches
			// a 'started' or 'errored' state.
			self.on('status-change', function _ (oldStatus, newStatus, error) {
				...
				setTimeout(() => callback.call(self, error || null, self));
			});
		}

		// Handle case when we are already in the process of starting the process.
		if (self.status === 'starting') {
			return self;
		}

		// We can only start from these states.
		if (self.status !== 'created' && self.status !== 'stopped' && self.status !== 'errored') {
			error = new Error('process cannot be started from a "' + self.status + '" state');
			self.emit('error', error);
