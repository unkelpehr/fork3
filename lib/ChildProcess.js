...
const
	...
	starterPath = path.join(__dirname, 'starter.js');
...
class ChildProcess extends EventEmitter {
	...
	start (callback, error) {
		...
		// 'FORK2'-property so that the starter module
		// know which module to execute.
		self.options.env.FORK2 = self.modulePath;

		// Fork!
		self.process = fork(starterPath, self.args, self.options);
		...
		if (callback) {
			self.once('status-change', function (oldStatus, newStatus, error) {
				console.log('DSDSFSDF', oldStatus, newStatus);
	...
	send (eventName) {
		...
		// Continue this invocation when the process has started or
		// we are in the process of shutting the process down and the event is a part of that.
		if (self.status !== 'started' && (self.status !== 'stopping' && eventName === 'restart')) {
